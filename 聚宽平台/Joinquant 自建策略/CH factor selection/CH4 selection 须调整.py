...
def CH4(stocks,begin,end,rf):
    ...
    sel_codes = list(df_13['code'])
    TR_lm_sum = 0
    TR_ly_sum = 0
    TR_sum = list(zeros(len(df_13.TR.values)))
   
    for i in range(-1,-253,-1):

        df_131= get_fundamentals(q,getDay(begin,i))
        df_131 = df_131[df_131.code.isin(sel_codes)]
        sel_codes2 = list(df_131['code'])

        df_131['codes'] = sel_codes2
        df_131['codes'] = df_131['codes'].astype('category')
        df_131['codes'].cat.reorder_categories(sel_codes,inplace = True)
        df_131.sort('codes',inplace = True)
        df_131.set_index(['codes'])
        TR_sum += list_add(list(df_131['TR']),TR_sum)
    ...
    T_df = pD.DataFrame({'code':sel_codes, 'TR': list_divide(TR_lm_sum,TR_ly_sum)},columns =['code','TR'])
    MAX = max((T_df['TR']).replace(np.inf,0))      
    T_df =T_df.replace(np.inf, MAX).fillna(value=mean(T_df['TR'])).sort('TR',ascending =False)  
       #if len(df_ly) ==0:
       #df_1y['TR']=1
        #if len(df_lm) ==0:
            #df_lm = df_1
    ...
    #求因子的值
    PMO= sum(df_4[T3].T)/len(T3) - sum(df_4[T1])/len(T1)
