...
def CH4(stocks,begin,end,rf):
    ...
    sel_codes = df_14.code
    df_15= pd.DataFrame({'code':sel_codes})
    ...
    for i in range(-1,-253,-1):
        ...
        if len(df_131) ==0:
            df_131 = pd.DataFrame({'code':sel_codes, 'TR':default},columns = ['code','TR'] )
        else:
    ...
    T_df = pd.DataFrame({'code':sel_codes, 'TR': list_divide(list(TR_lm_sum),list(TR_ly_sum))},columns =['code','TR'])
    MAX = max((T_df['TR']).replace(np.inf,0))      
    T_df =T_df.replace(np.inf, MAX).sort('TR',ascending =False)  
